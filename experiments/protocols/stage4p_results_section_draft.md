...
issue in Trial 4 repeat2-5. Stage 4-X2 completes the missing single-goal
baselines. Future Results should report:

| Protocol | Method | Strict-valid count |
| --- | --- | ---: |
| single-goal mission, `goal_repeat=1` | `windlevel_s085` | `26/30` |
| single-goal mission, `goal_repeat=1` | `risk_adapter_v1` | `25/30` |
| single-goal mission, `goal_repeat=1` | `risk_adapter_v21` | `25/30` |
| single-goal mission, `goal_repeat=1` | `fixed_s085` | `22/30` |
| single-goal mission, `goal_repeat=1` | `original` | `21/30` |
...
The Results narrative should describe `windlevel_s085` as the strongest
completed single-goal method at `26/30`, with `risk_adapter_v1` and
`risk_adapter_v21` tied at `25/30`. It should state that `fixed_s080` remains
the strongest evaluated goal-reissue stress method. `risk_adapter_v21` should
not be described as the best single-goal method, as beating all baselines, or
as a cross-protocol final winner.
...
- Stage 4-V4 provides the `risk_adapter_v21` single-goal result:
  `risk_adapter_v21` `25/30` under `goal_repeat=1`.
- Stage 4-X2 provides the completed single-goal baseline comparison:
  `windlevel_s085` `26/30`, `risk_adapter_v1` `25/30`,
  `risk_adapter_v21` `25/30`, `fixed_s085` `22/30`, and `original`,
  `fixed_s080`, and `risk_adapter_v2` each `21/30`.
- Stage 4-X1 provides the corrected `risk_adapter_v21` goal-reissue stress
  result: `20/30` under `goal_repeat=10`.
- Future Results should separate completed single-goal mission results
  (`windlevel_s085` `26/30`, `risk_adapter_v1` `25/30`,
  `risk_adapter_v21` `25/30`) from goal-reissue stress results
  (`fixed_s080` `24/30`, `risk_adapter_v1` `23/30`,
  `risk_adapter_v21` `20/30`).
- `risk_adapter_v21` can be described as competitive with `risk_adapter_v1`
  and improved over `original`, `fixed_s080`, `fixed_s085`, and
  `risk_adapter_v2` in the completed single-goal aggregate.
- `windlevel_s085` is the strongest completed single-goal aggregate method and
  should be treated as a strong heuristic baseline.
...
- Do not claim learned governors dominate simple heuristic baselines.
...
- Stage 4-X2 supersedes wording that described `risk_adapter_v21` as the
  strongest single-goal method before `original`, `fixed_s085`,
  `windlevel_s085`, and `risk_adapter_v1` were added under `goal_repeat=1`.
