...
def build_paper_summary(success_rows, aggregate_rows, failure_rows):
    ...
    return "\n".join(
        [
            ...
            "## Failure-Mode Caveat",
            "",
            "Failure modes are mixed. Some failures are `command_saturation_before_nan`, some are `state_divergence_before_command_nan`, and some are `strict_safety_no_nan` or `target_error_only`.",
            "",
            "Transient `command_saturation_without_divergence` is not a failure by itself. `swing_angle_deg >= 30` is warning-only; paper-facing strict-valid / `label_strict_invalid` remains the primary success metric.",
            "",
