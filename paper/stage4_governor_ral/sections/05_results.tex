...
goal-reissue stress. This split is important because the same method can behave
differently when the system receives one goal command versus repeated
post-arrival or reference-update stimuli. We therefore report strict-valid
counts separately for the single-goal mission protocol (goal repeat = 1) and
the goal-reissue stress protocol (goal repeat = 10), rather than collapsing the
two settings into a single combined aggregate.

In the single-goal mission protocol, the heuristic Wind-Level 0.85 is strongest
in the evaluated set, achieving 26/30 strict-valid runs. The learned /
risk-conditioned methods Risk Adapter v1 and Risk Adapter v2.1 are also
competitive, each achieving 25/30 strict-valid runs. In the goal-reissue stress
protocol, the ranking changes: Fixed Scale 0.80 is strongest with 24/30
strict-valid runs, Risk Adapter v1 follows with 23/30, and Risk Adapter v2.1
drops to 20/30.
...
Balanced robustness is evaluated from the complete cross-protocol matrix. The
protocol oracles are the highest observed method in each protocol: Wind-Level
0.85 with 26/30 in the single-goal mission protocol and Fixed Scale 0.80 with
24/30 in the goal-reissue stress protocol. Protocol regret measures how far
each method falls below these observed oracles.
...
23/30, and the lowest total regret of 2. These criteria motivate centering
...
The specialist baselines expose the trade-off hidden by a single protocol.
Wind-Level 0.85 reaches the single-goal oracle with 26/30 but falls to
...
Protocol-split success counts show which methods are stronger, but not how
invalid runs occur. We therefore use invalid-only failure groups as a
diagnostic view. These labels are failure-analysis categories rather than
definitive root-cause attribution; the paper-facing success metric remains
strict-valid count.
...
supports the reframed paper claim that aggregate strict-valid counts alone are
insufficient for interpreting robustness.

\subsection{Representative Trace Analysis}

Representative traces provide a case-study view of the same trade-offs. In
Trial 4 under goal-reissue stress, several Risk Adapter v2.1 failures occur
...
safer, and delayed post-arrival divergence can still occur under goal repeat =
1.
...
\begin{figure*}[t]
  ...
  bottleneck behavior. The figure keeps only key speed, swing, UAV XY
  displacement, and governor-scale signals for readability. Full trace details
  are available in supplementary / archived trace assets. The traces are
  qualitative mechanism evidence, not proof of general causality.}
...
\begin{itemize}
  ...
  \item Protocol split and invalid-only failure groups are needed to interpret
  robustness; a single combined aggregate would hide specialist
  trade-offs.
\end{itemize}
