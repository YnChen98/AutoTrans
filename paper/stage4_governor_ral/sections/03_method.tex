...
payload-aware planner, a payload MPC, and an SO3 controller
...
references while accounting for the load, and the SO3 controller provides the
...
\texttt{speed\_scale} and \texttt{acceleration\_scale}. These signals are
...
\begin{figure}[t]
  ...
  planner, payload MPC, and SO3 controller remain unchanged, while an external
  governor adapts execution aggressiveness through \texttt{speed\_scale} and
  \texttt{acceleration\_scale}. The governor is empirical and is not a
...
\texttt{risk\_score\_3s} and \texttt{risk\_score\_5s}. The adapter subscribes
to odometry, payload odometry, wind annotation, goal, and trajectory-context
streams, builds early episode features, and evaluates exported JSON
\texttt{LogisticRegression} models trained with
\texttt{label\_strict\_invalid} as the positive class. The audited local JSON
...
\texttt{early\_3s} features, while the 5 s model uses 27 features spanning the
same base fields plus \texttt{early\_3s} and \texttt{early\_5s} dynamics; the
full feature schema is kept for supplementary material. Unavailable scores are
published as \texttt{-1} and do not trigger risk reductions. These values are
used as empirical strict-invalid warning scores, not as calibrated physical
probabilities; the current implementation does not include confidence or OOD
rejection, and inference latency has not been measured separately from the
...
\subsection{Current Protagonist: \texttt{risk\_adapter\_v1}}

The current paper protagonist is \texttt{risk\_adapter\_v1}, the balanced
learned / risk-conditioned execution governor. Its policy settings are:

\begin{itemize}
  \item \texttt{risk\_threshold\_3s=0.5}
  \item \texttt{risk\_threshold\_5s=0.5}
  \item \texttt{hard\_threshold\_5s=0.7}
  \item \texttt{soft\_scale\_3s=0.75}
  \item \texttt{soft\_scale\_5s=0.65}
  \item \texttt{hard\_scale\_5s=0.60}
  \item \texttt{scale\_rate\_limit\_per\_sec=0.5}
\end{itemize}

\texttt{risk\_adapter\_v1} is selected as the tentative balanced protagonist
because it gives the strongest observed cross-protocol balance in the current
evaluation. It achieves 25/30 strict-valid runs in the single-goal mission
protocol and 23/30 in the goal-reissue stress protocol, yielding the highest
mean valid count (24.0/30), highest worst-protocol valid count (23/30), and
lowest total regret (2) relative to the observed protocol oracles.

The later \texttt{risk\_adapter\_v21} variant remains important, but it should
not be presented as the final method. It ties \texttt{risk\_adapter\_v1} in the
...
while \texttt{risk\_adapter\_v1} is the current balanced learned /
...
learned / risk-conditioned variants: \texttt{original}, \texttt{fixed\_s085},
\texttt{windlevel\_s085}, \texttt{fixed\_s080},
\texttt{risk\_adapter\_v1}, \texttt{risk\_adapter\_v2}, and
\texttt{risk\_adapter\_v21}. \texttt{windlevel\_s085} is the single-goal
specialist heuristic. \texttt{fixed\_s080} is the goal-reissue stress
specialist static frontier. \texttt{risk\_adapter\_v1} is the balanced learned
/ risk-conditioned protagonist, while \texttt{risk\_adapter\_v21} is a strong
nominal variant that is weaker under stress.
